...
namespace ScreenTask
{
    public partial class frmMain : Form
    {
        ...
        {
            // Get all monitors 
            List<UserMonitor> monitorNames = Screen.AllScreens
                .OrderByDescending(m => m.Primary)
                .Select(m => new UserMonitor
                {
                   DisplayValue  = (m.Primary ? m.DeviceName + " (Основной)" : m.DeviceName),
                   DeviceName = m.DeviceName
                })
                .ToList();
            // Bindidng monitors to ComboBox
            comboBox_Monitors.DisplayMember = "DisplayValue";
        ...
        private void TakeScreenshot(bool captureMouse, string name)
        {
            ...
            Rectangle bounds = Screen.AllScreens.First(m => m.DeviceName.Equals(nameSelectedMonitor)).Bounds;
